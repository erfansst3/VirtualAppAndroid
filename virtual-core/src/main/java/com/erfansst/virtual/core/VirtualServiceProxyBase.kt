...
abstract class VirtualServiceProxyBase(private val slot:Int):Service(){
...
override fun onStartCommand(intent:Intent?,flags:Int,startId:Int):Int{val i=intent?:return START_NOT_STICKY;val t=runCatching{target(i)}.getOrNull()?:return START_NOT_STICKY;return t.onStartCommand(clean(i),flags,startId)}
override fun onBind(intent:Intent?):IBinder?{val i=intent?:return null;return runCatching{target(i)?.onBind(clean(i))}.getOrNull()}
