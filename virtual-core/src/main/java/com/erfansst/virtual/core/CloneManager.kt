...
import java.io.File
data class CloneInfo(val packageName:String,val cloneId:Int,val apk:File,val dataDir:File)
...
File(app.appInfo.sourceDir).copyTo(apk,true)
val data=File(dir,"data");data.mkdirs()
return CloneInfo(app.packageName,id,apk,data)
}
fun list():List<CloneInfo>{
if(!root.exists())return emptyList()
return root.listFiles().orEmpty().mapNotNull{d->
val n=d.name.lastIndexOf('_')
if(n<1)return@mapNotNull null
val id=d.name.substring(n+1).toIntOrNull()?:return@mapNotNull null
val p=d.name.substring(0,n);val apk=File(d,"base.apk");if(!apk.exists())null else CloneInfo(p,id,apk,File(d,"data"))
}}
}