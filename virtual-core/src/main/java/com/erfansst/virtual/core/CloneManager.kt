...
val apk=File(dir,"base.apk");File(app.appInfo.sourceDir).copyTo(apk,true);apk.setReadOnly()
val splitDir=File(dir,"splits");splitDir.mkdirs()
val splits=app.appInfo.splitSourceDirs.orEmpty().mapIndexed{n,p->File(splitDir,"split_$n.apk").also{File(p).copyTo(it,true);it.setReadOnly()}}
