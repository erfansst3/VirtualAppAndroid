...
val p=Proxy.newProxyInstance(iface.classLoader,arrayOf(iface),handler(base));f.set(singleton,p)
}
...
runCatching{val pm=context.packageManager;val m=pm.javaClass.getDeclaredField("mPM");m.isAccessible=true;m.set(pm,p)}
}
...
when{
m.name.startsWith("getApplicationInfo")||m.name.startsWith("getPackageInfo")->spoof(m.invoke(base,*rewritten),s)
m.name=="getPackageUid"&&a.firstOrNull()==s.appInfo.packageName->Process.myUid()
...
is PackageInfo->PackageInfo().apply{packageName=s.appInfo.packageName;applicationInfo=spoof(applicationInfo?:s.appInfo,s) as ApplicationInfo}
is ActivityInfo->ActivityInfo(x).apply{applicationInfo=spoof(applicationInfo?:s.appInfo,s) as ApplicationInfo}
is ServiceInfo->ServiceInfo(x).apply{applicationInfo=spoof(applicationInfo?:s.appInfo,s) as ApplicationInfo}
is ProviderInfo->ProviderInfo(x).apply{applicationInfo=spoof(applicationInfo?:s.appInfo,s) as ApplicationInfo}
